...
class Dropdown4 extends HTMLElement {
  ...
  open() {
    ...
    //有陰影的下拉選單
    if ($(this).siblings('.dropdown-shadow').length > 0) {
      const shadow = $(this).siblings('.dropdown-shadow')[0]
      shadow.setAttribute('d4-status', 'open');
      shadow.style.cssText = `
      height: ${dropdownHeight() + 30 }px;
      `;
    }

    if ($(this).siblings('.dropdown-shadow').length > 0 && $(this).parent('.dropdown-box').hasClass('separate'))  {
      const shadow = $(this).siblings('.dropdown-shadow')[0]
      shadow.style.cssText = `
      height: ${dropdownHeight() + 10 }px;
      `;
    }
  ...
  close() {
    ...
    if ($(this).siblings('.dropdown-shadow').length > 0) {
      const shadow = $(this).siblings('.dropdown-shadow')[0]
      shadow.setAttribute('d4-status', 'close');
      shadow.style.cssText = 'height: 0px;';
    }
    return this;
  }
  scrollTo(scroll) {
